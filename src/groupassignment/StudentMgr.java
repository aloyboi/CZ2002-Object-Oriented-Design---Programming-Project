...
public class StudentMgr extends AllTXT {
					...
					if (j.getcourseCode1().equals(i.getCourseCode())) {
						for (Lesson l: lessonlist) {
							if (l.getcourseCode1().equalsIgnoreCase(j.getcourseCode1()) && l.getIndexNum().equalsIgnoreCase(i.getIndexNumber())) {						
								//System.out.println(i.getCourseCode() + "       " + j.getcourseName1() + "             " + i.getIndexNumber() + "         " + l.getLessonDay()
								//+ "         "+ l.getLessonStartTimeStr()+ "         "+l.getLessonEndTimeStr()+ "         "+l.getLessonLoc()+ "         "+l.getLessonType()+ "         "+j.getExamDate());
	...
	public static int checkClash(String indexNum,Student s,String courseID) throws ParseException, IOException {
		...
		for (Lesson p: lessonlist) { //get all lessons for that particular index
		...
		for (Course i: courselist) { //get the course details
	...
	public static void changeCourseIndex(Student student, String currentIndex, String newIndex,String courseID) throws Exception {
		Index updateindex = new Index(); //for old index
		Index updateindex1 = new Index(); //for new index
		int count=0;
		int count1=0;
	...
	public static void swapStudent(Student s1, String student2, String index,String index2, String courseID) throws Exception {
		...
		for (Student l: studentlist) { //check if 2nd student exists
		...
		for (Index i: indexlist) { //check coursecode & index validity
		...
		for (RegisteredCourses j: registeredCourseslist) { //check if both are registered for the same index
